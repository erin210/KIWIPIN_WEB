...
$(document).ready(function () {
  ...
  $(document).on('click', function (event) {
    ...
    if (!$(event.target).closest('#sidr').length && !$(event.target).closest('#menu-toggle').length) {
      $('#menu-toggle').removeClass('active'); // 延遲隱藏選單的動作
...
$(document).on('click', '.navLinkIndex', function (event) {
  ...
  var targetElement = $("#" + listID);

  var scrollToTarget = function scrollToTarget(element) {
    var targetPosition = element.offset().top - 20;
    $('html, body').animate({
  ...
  if (targetElement.length) {
    ...
    window.history.pushState(null, null, "#".concat(listID));
  } else {
    window.location.href = "/#".concat(listID);
  }
}); //加 active

var navList_a = '#sidr a';
...
$(document).on('click', '#langListItem li', function () {
  ...
  console.log(selectLang); // langDefault = selectLang;
